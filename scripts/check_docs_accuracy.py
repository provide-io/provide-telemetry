...
def _claim_violations(path: Path, content: str) -> list[str]:
    violations: list[str] = []
    if "no-op tracing/metrics" in content or "no-op tracing and metrics" in content:
        violations.append(
            f"{path}: fallback wording is inaccurate; tracing is no-op fallback, metrics are in-process fallback wrappers"
        )
