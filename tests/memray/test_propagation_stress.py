...
@pytest.mark.memray
@pytest.mark.slow
def test_propagation_stress(
    memray_output_dir: Path,
    memray_baseline: dict[str, int],
    assert_allocation_within_threshold: Callable[..., None],
    project_root: Path,
) -> None:
    """Stress test propagation context parsing with memray profiling."""
    script_path = project_root / "scripts" / "memray" / "memray_propagation_stress.py"
    output_bin = memray_output_dir / "memray_propagation_stress.bin"
    ...
    baseline = memray_baseline.get("propagation_total_allocations")
    assert_allocation_within_threshold(baseline, total_allocations, "propagation")
