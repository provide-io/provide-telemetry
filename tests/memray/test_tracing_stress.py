...
"""Memray stress test for tracing decorator context path."""
...
@pytest.mark.memray
@pytest.mark.slow
def test_tracing_stress(
    memray_output_dir: Path,
    memray_baseline: dict[str, int],
    assert_allocation_within_threshold: Callable[..., None],
) -> None:
    """Stress test tracing decorator with memray profiling."""
    script_path = Path(__file__).parent.parent.parent / "scripts" / "memray" / "memray_tracing_stress.py"
    output_bin = memray_output_dir / "memray_tracing_stress.bin"
    ...
    baseline = memray_baseline.get("tracing_total_allocations")
    assert_allocation_within_threshold(baseline, total_allocations, "tracing")
