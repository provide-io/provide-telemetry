...
class TestHalfOpenProbe:
    ...
    def test_concurrent_probe_rejected_fail_closed(self) -> None:
        """Concurrent callers during a half-open probe are rejected when fail_open=False."""
        reset_resilience_for_tests()
        set_exporter_policy("logs", ExporterPolicy(fail_open=False, timeout_seconds=10.0))
        # Force threshold reached + half-open probing already in progress
        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD
            resilience_mod._open_count["logs"] = 1
        ...
        with pytest.raises(TimeoutError, match="circuit breaker open"):
            run_with_resilience("logs", operation)

        assert not fn_called

    def test_concurrent_probe_rejected_fail_open(self) -> None:
        """Concurrent callers during a half-open probe return None when fail_open=True."""
        reset_resilience_for_tests()
        set_exporter_policy("logs", ExporterPolicy(fail_open=True, timeout_seconds=10.0))
        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD
            resilience_mod._open_count["logs"] = 1
            resilience_mod._half_open_probing["logs"] = True
        ...
        result = run_with_resilience("logs", operation)
...
class TestCBMutationKills:
    """Tests targeting specific surviving mutants in _check_circuit_breaker, _record_attempt_*, get_circuit_state."""

    def test_probe_success_resets_consecutive_timeouts_to_zero_not_one(self) -> None:
        """Kill mutmut_4: _consecutive_timeouts = 1 instead of 0 after half-open probe success.

        After a probe succeeds, adding 2 more timeouts should leave count at 2 (< threshold=3) → closed.
        With mutation (reset to 1): 1+2=3 ≥ threshold → open.
        """
        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD
            resilience_mod._open_count["logs"] = 1
            resilience_mod._circuit_tripped_at["logs"] = time.monotonic() - _CIRCUIT_BASE_COOLDOWN * 4

        set_exporter_policy("logs", ExporterPolicy(timeout_seconds=0.0, retries=0, fail_open=True))
        run_with_resilience("logs", lambda: None)  # probe succeeds

        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] += 2  # simulate 2 subsequent timeouts

        state, _, _ = get_circuit_state("logs")
        assert state == "closed", f"Expected closed (reset to 0, 0+2=2 < 3); got {state}"

    def test_record_attempt_failure_increments_open_count_from_existing_value(self) -> None:
        """Kill mutmut_11: _open_count = 1 instead of += 1 on consecutive-timeout trip.

        With open_count=5 and one more timeout: += 1 → 6, but = 1 → 1.
        """
        from unittest.mock import patch

        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD - 1
            resilience_mod._open_count["logs"] = 5

        set_exporter_policy("logs", ExporterPolicy(timeout_seconds=1.0, retries=0, fail_open=True))
        with patch.object(resilience_mod, "_run_attempt_with_timeout", side_effect=TimeoutError("timed out")):
            run_with_resilience("logs", lambda: None)

        with resilience_mod._lock:
            oc = resilience_mod._open_count["logs"]
        assert oc == 6, f"Expected 6 (5+1); got {oc} — mutation would give 1"

    def test_get_circuit_state_uses_exponential_not_linear_cooldown(self) -> None:
        """Kill mutmut_13: 2**N vs 2*N in get_circuit_state cooldown.

        open_count=3: exponential 30*8=240s, linear 30*6=180s. Assert > 200s.
        """
        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD
            resilience_mod._open_count["logs"] = 3
            resilience_mod._circuit_tripped_at["logs"] = time.monotonic()

        state, _, remaining = get_circuit_state("logs")
        assert state == "open"
        assert remaining > 200.0, f"Expected > 200s (exp. 2^3=8 → 240s); got {remaining:.1f}s"

    def test_check_circuit_breaker_cooldown_uses_multiplication_not_division(self) -> None:
        """Kill mutmut_8: BASE * 2**N vs BASE / 2**N in _check_circuit_breaker.

        open_count=2, tripped 8s ago: correct=120s (open), mutated=7.5s (allows probe).
        """
        with resilience_mod._lock:
            resilience_mod._consecutive_timeouts["logs"] = _CIRCUIT_BREAKER_THRESHOLD
            resilience_mod._open_count["logs"] = 2
            resilience_mod._circuit_tripped_at["logs"] = time.monotonic() - 8.0
        ...
        run_with_resilience("logs", _op)
        assert not fn_called["v"], "CB open (120s not elapsed); probe must not run"
