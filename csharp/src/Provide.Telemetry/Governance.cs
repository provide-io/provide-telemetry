...
public static class Receipts
{
    ...
    private static readonly List<RedactionReceipt> Emitted = new();
    private static readonly object Gate = new();
    ...
    internal static void Record(string fieldPath, string action, object? original)
    {
        lock (Gate)
        {
            if (!_enabled) return;
            ...
            var originalText = original switch
            {
                null => "",
                string s => s,
                _ => Convert.ToString(original, System.Globalization.CultureInfo.InvariantCulture) ?? "",
            };
            var originalHash = Convert.ToHexString(
                System.Security.Cryptography.SHA256.HashData(
                    System.Text.Encoding.UTF8.GetBytes(originalText))).ToLowerInvariant();
            var hmac = "";
            if (!string.IsNullOrEmpty(_signingKey))
            {
                var payload = $"{receiptId}|{timestamp}|{fieldPath}|{action}|{originalHash}";
                using var mac = new System.Security.Cryptography.HMACSHA256(
                    System.Text.Encoding.UTF8.GetBytes(_signingKey));
                hmac = Convert.ToHexString(
                    mac.ComputeHash(System.Text.Encoding.UTF8.GetBytes(payload))).ToLowerInvariant();
            }
            Emitted.Add(new RedactionReceipt
            {
                ReceiptId = receiptId,
                FieldPath = fieldPath,
                Action = action,
    ...
    {
        lock (Gate) { return Emitted.ToList(); }
    }
