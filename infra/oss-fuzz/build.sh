...
################################################################################
#
# Native Go 1.18+ fuzz targets from go/fuzz_test.go for ClusterFuzz / OSS-Fuzz.
# Validated against base-builder-go compile_native_go_fuzzer conventions
# (see projects/go-cmp, projects/opentelemetry-go).

cd "$SRC/provide-telemetry/go"

# go-118-fuzz-build fails without this when the source tree is not a full git checkout
# (OSS-Fuzz mounts / local helper builds).
