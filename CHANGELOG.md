...
- **Repeated flushes cannot exhaust the process's threads** — the bounded-drain helper abandons its worker at the deadline, which is fine for `shutdown_telemetry` (once per process) and not for `flush_telemetry` (documented for per-request use). Against an unreachable collector every call stranded another thread in the exporter's retry loop until interpreter exit. Abandoned workers are now capped at 8; past that a drain declines immediately and reports failure rather than adding to the pile.
