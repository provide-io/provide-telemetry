...
- Python runs mutmut and fails on any survivor, timeout, suspicious, or no-tests result — the 95% score floor is an extra guard, not the bar; Go requires both 100% gremlins efficacy and 100% mutant coverage; TypeScript uses Stryker with a 95% core break threshold plus an 80% OTLP transport ratchet; Rust requires a 100% cargo-mutants kill rate across eight blocking shards whenever Rust implementation or test code changes. C# runs Stryker.NET 4.16 over both packages with a break threshold of 78, against a measured 79.14% (1318 killed of 1678 scored, 2026-08-09) — the honest baseline for a gate that is one release old, not a target. Its surviving mutants are enumerated in `csharp/stryker-config.json`.
