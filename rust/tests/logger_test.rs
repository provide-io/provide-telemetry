...
use provide_telemetry::{buffer_logger, get_logger, null_logger, trace, Logger};

/// Minimal subscriber that records the level of every event emitted on the
/// current thread. Used with `tracing::subscriber::with_default` to verify
/// that `Logger::log` routes to the correct tracing macro for each level.
struct LevelCapture(Arc<Mutex<Vec<String>>>);
...
impl tracing::Subscriber for LevelCapture {
    ...
    fn event(&self, event: &tracing::Event<'_>) {
        self.0
            .lock()
            .expect("level capture lock poisoned")
