...
- Each language has one obvious "local dev" path and one obvious "production"
  path.
...
- Level filtering, consent, sampling, schema, PII, backpressure, and fail-open
...
- Users can inspect the active config, runtime mode, provider installation
  state, and health snapshot.
...
- Language-specific tests verify idiomatic facades without weakening the shared
  contract.
...
- Public APIs do not split into "real" and "stub" behavior without being
  obvious.
...
6. Are known differences documented as intentional and idiomatic?
...
- `Core guaranteed behavior`
  Behavior that must match across all languages.
- `Idiomatic language difference`
  Syntax or shape differences that preserve the same semantics.
- `Known gap`
  Current divergence that is not yet resolved and must be tracked.
